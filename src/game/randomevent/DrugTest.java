...
/**
 * A random drug test event that is triggered weekly.
 * Every athlete in the player's team is tested for drug use.
 * If any athletes are found to have used steroids, the game ends.
 * 
...
public class DrugTest extends RandomEvent {
	...
	/**
	 * Tests every athlete in the player's team for steroid use.
	 * If any athletes have used steroids, the game location is changed to the end location
	 */
	@Override
	protected void occur() {
		...
		for (Athlete athlete: reserveAthletes) {
			if (athlete.getHasUsedSteroids()) {
				passedDrugTest = false;
				break;
			}
	...
	protected float getProbability() {
		...
		return 0.1f * tournamentProgression;
	}
}
